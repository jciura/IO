...
public class DataInitializer implements CommandLineRunner {
    ...
    public DataInitializer(UserRepository userRepository, ClassroomRepository classroomRepository, ClassSessionRepository classSessionRepository, SoftwareRepository softwareRepository, CourseRepository courseRepository) {
    ...
    public void run(String... args) {
        ...
        Course course = new Course("Intro to Test", lecturer, classRep);
        courseRepository.save(course);

        lecturer.addCourse(course);
        classRep.addCourse(course);
        userRepository.save(lecturer);
        userRepository.save(classRep);

        ClassSession classSession = new ClassSession("Test", course, classroom, LocalDateTime.now(), 90);
        course.addSession(classSession);
